...
github_repo = st.text_input(
    "Enter GitHub repo (e.g. openai/openai-python)",
    placeholder="owner/repo-name"
)

analyze_options = st.multiselect(
    "What to analyze?",
    ["Full AI Report", "Commits", "Issues", "Contributors", "Languages"],
    default=["Full AI Report"]
)

if st.button("Analyze Repository"):
    import os 
    st.write(f"Token loaded: {bool(os.getenv('GITHUB_TOKEN'))}")
    st.write(f"Token preview: {os.getenv('GITHUB_TOKEN', 'NOT FOUND')[:15]}")
    if not github_repo or "/" not in github_repo:
        st.warning("Enter a valid repo in owner/repo format.")
    else:
        ...
        with st.spinner("Fetching repo data..."):
            if "Full AI Report" in analyze_options:
                report = analyze_github_repo(llm, github_repo)
                st.markdown(report)

            if "Commits" in analyze_options:
                commits = get_recent_commits(github_repo)
                with st.expander("Recent Commits"):
                    for c in commits:
                        st.write(f"**{c['author']}** — {c['message'][:80]} `{c['date'][:10]}`")

            if "Issues" in analyze_options:
                issues = get_open_issues(github_repo)
                with st.expander("Open Issues"):
                    for i in issues:
                        st.write(f"- {i['title']} {i['labels']}")

            if "Contributors" in analyze_options:
                contributors = get_contributors(github_repo)
                with st.expander("Top Contributors"):
                    for c in contributors:
                        st.write(f"**{c['login']}** — {c['contributions']} commits")

            if "Languages" in analyze_options:
                langs = get_languages(github_repo)
